...
<body>
<a href="customer_home.php"><button style="height: 40px; width: 10rem; background-color:#00cc66;cursor: pointer;
...
$sql= $connect->query("SELECT * FROM customers where username='$user'");
$row= mysqli_fetch_array($sql);
if (password_verify($old_pass, $row['password'])) {
    $hashedPassword=password_hash($new_pass,PASSWORD_DEFAULT);
    $query="UPDATE `customers` SET `password`='$hashedPassword' where username='$user'";
    echo $query;
